...
export const createOwnerRestaurant = async (req:AuthRequest, res:Response):Promise<void> => {
    try {
        ...
        if(existing){
            res.status(400).json({message:"You already have restaurant registered"})
            return;
        }
...
export const updateBookingStatus = async (req:AuthRequest, res:Response):Promise<void> => {
    try {
        ...
        if(!status || !["confirmed","cancelled","completed"].includes(status)){
            res.status(400).json({message: "Please include valid booking status"})
            return;
        }
        ...
        //Verify Booking belongs to Onwer's restaurant
